// Package kuznyechik реализует блочный шифр «Кузнечик» (ГОСТ Р 34.12-2015).
//
// Шифр работает с 128-битными блоками и 256-битным ключом.
// Реализация основана на табличной оптимизации (combined S-box + L-transform lookup tables)
// из cppcrypto 0.15 (public domain).
//
// Ссылки:
//   - ГОСТ Р 34.12-2015 «Информационная технология. Криптографическая защита информации.
//     Блочные шифры» (https://tc26.ru/standard/gost/GOST_R_3412-2015.pdf)
...
// NewCipher создаёт шифр «Кузнечик» с 256-битным ключом.
// Реализует интерфейс crypto/cipher.Block.
func NewCipher(key []byte) (cipher.Block, error) {
	if len(key) != KeySize {
		return nil, fmt.Errorf("kuznyechik: invalid key size %d, must be %d", len(key), KeySize)
	}
...
// ZeroKey обнуляет раундовые ключи в памяти. После вызова шифр нельзя использовать.
func (k *kuznyechikCipher) ZeroKey() {
	for i := range k.erk {
		k.erk[i][0] = 0
		k.erk[i][1] = 0
	}
