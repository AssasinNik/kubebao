...
### 5.2.1 Rancher Desktop — загрузка образов в кластер

> Rancher Desktop использует **k3s** внутри Lima VM со своим Docker-демоном. Образы, собранные командой `docker build` на хосте macOS, **не видны** Kubernetes-ноде — их нужно явно загрузить в VM.

**Шаг 1.** Убедитесь, что Rancher Desktop запущен с Container Engine: **dockerd (moby)**:

```
Rancher Desktop → Preferences → Container Engine → dockerd (moby)
```

**Шаг 2.** Загрузите все 4 образа в Docker-демон внутри Lima VM:

```bash
docker save kubebao/kubebao-kms:dev      | rdctl shell -- docker load
docker save kubebao/kubebao-csi:dev      | rdctl shell -- docker load
docker save kubebao/kubebao-operator:dev | rdctl shell -- docker load
docker save kubebao/kubebao-ui:dev       | rdctl shell -- docker load
```

**Шаг 3.** Проверьте, что образы доступны внутри VM:

```bash
rdctl shell -- docker images | grep kubebao
# REPOSITORY                  TAG   IMAGE ID       CREATED         SIZE
# kubebao/kubebao-kms         dev   abc123...      1 minute ago    38MB
# kubebao/kubebao-csi         dev   def456...      1 minute ago    31MB
# kubebao/kubebao-operator    dev   ghi789...      1 minute ago    36MB
# kubebao/kubebao-ui          dev   jkl012...      1 minute ago    40MB
```

> **Важно:** при каждой пересборке образов (после изменений в коде) нужно повторить шаг 2. Иначе кластер будет использовать старую версию.

> **Minikube:** вместо `rdctl shell` используйте `minikube image load kubebao/kubebao-kms:dev`
...
> **Rancher Desktop / Minikube (локальные образы):** обязательно задайте `pullPolicy=Never` и `registry=""`, иначе кластер попытается скачать образы с Docker Hub и получит `ErrImagePull`.
...
Флаги:
- `global.image.pullPolicy=Never` — использовать только локально загруженные образы, не обращаться в registry
- `global.image.registry=""` — пустой registry, имя образа = `repository:tag`
- `csi.driver.install=false` — CSI Driver уже установлен на шаге 5.3, не переустанавливать
...
### 6.1 Проверка, что UI запущен

После установки KubeBao (раздел 5.4) проверьте статус пода UI:
...
Если под в состоянии `ErrImageNeverPull` или `ErrImagePull` — вернитесь к разделу 5.2.1 и загрузите образы.

Посмотрите логи запуска UI:

```bash
kubectl logs -n kubebao-system -l app.kubernetes.io/component=ui
# {"level":"info","msg":"KubeBao UI starting","addr":":8443","openbao":"http://openbao.openbao.svc.cluster.local:8200"}
```

### 6.2 Доступ через port-forward (для локальной разработки)
...
Оставьте этот терминал открытым. Откройте в браузере: **http://localhost:8443**

Если порт 8443 уже занят, используйте другой:

```bash
kubectl port-forward svc/kubebao-ui 9090:8443 -n kubebao-system
# Откройте: http://localhost:9090
```

Проверьте, что UI отвечает:
...
**Авторизация:** UI требует OpenBao-токен для входа. На странице логина введите токен, настроенный в переменной `OPENBAO_TOKEN`. Для dev-окружения — `root`, для production — сервисный токен (см. 6.3). Токен хранится в `sessionStorage` браузера и отправляется в заголовке `X-Token` при каждом API-запросе.

### 6.3 Настройка OpenBao-токена для UI

UI использует токен OpenBao для операций с ключами (ротация, чтение метаданных ключа). Токен задаётся переменной окружения `OPENBAO_TOKEN`.

**Для dev/тестирования** — укажите root-токен при установке Helm:
...
**Для production** — создайте сервисный токен с минимальными правами:

```bash
# Убедитесь, что BAO_ADDR и BAO_TOKEN экспортированы (раздел 4.2)

# Создание политики только для UI
curl -s -X PUT "http://127.0.0.1:8200/v1/sys/policies/acl/kubebao-ui-policy" \
  -H "X-Vault-Token: $BAO_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "policy": "path \"secret/data/kubebao/*\" { capabilities = [\"create\",\"read\",\"update\",\"list\"] }\npath \"secret/metadata/kubebao/*\" { capabilities = [\"read\",\"list\"] }\npath \"secret/data/*\" { capabilities = [\"read\",\"list\"] }\npath \"secret/metadata/*\" { capabilities = [\"read\",\"list\"] }"
  }'

# Создание сервисного токена с TTL
curl -s -X POST "http://127.0.0.1:8200/v1/auth/token/create" \
  -H "X-Vault-Token: $BAO_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"policies":["kubebao-ui-policy"],"ttl":"720h","renewable":true}' | jq -r '.auth.client_token'
```

Запишите полученный токен и задайте его:

```bash
UI_TOKEN="hvs.XXXXX"   # токен из вывода выше

helm upgrade kubebao ./charts/kubebao \
  --namespace kubebao-system \
  --reuse-values \
  --set "ui.env[0].name=OPENBAO_TOKEN" \
  --set "ui.env[0].value=$UI_TOKEN"
```

После обновления под UI перезапустится с новым токеном:

```bash
kubectl rollout status deployment/kubebao-ui -n kubebao-system
```

### 6.4 Доступ через Ingress

> Для Ingress необходим установленный **Ingress Controller** (например, nginx). Для Rancher Desktop без Ingress Controller используйте port-forward (раздел 6.2).

**Проверьте наличие Ingress Controller:**

```bash
kubectl get pods -n ingress-nginx 2>/dev/null || \
kubectl get pods -n kube-system -l app.kubernetes.io/name=ingress-nginx 2>/dev/null || \
echo "Ingress Controller не установлен"
```
...
# ingress-nginx-controller     LoadBalancer   10.43.12.34    127.0.0.1     80:32080/TCP,443:32443/TCP
```

**Включение Ingress для UI при установке Helm:**
...
**Добавьте запись в `/etc/hosts` (для локальной разработки):**

```bash
# Узнайте внешний IP Ingress Controller
INGRESS_IP=$(kubectl get svc -n ingress-nginx ingress-nginx-controller \
  -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
echo "$INGRESS_IP  kubebao.local" | sudo tee -a /etc/hosts
```

> Для Rancher Desktop `EXTERNAL-IP` обычно `127.0.0.1`.
...
# kubebao-ui    nginx   kubebao.local   127.0.0.1   80      1m

kubectl describe ingress kubebao-ui -n kubebao-system
```

Откройте в браузере: **http://kubebao.local**

### 6.5 Обзор страниц UI

| Страница | Описание |
|---|---|
| **Overview** | Статус системы: OpenBao, KMS-провайдер, ключ, uptime. Графики Heap/Goroutines за последний час |
...
**Страница Keys — просмотр текущего ключа:**

1. Перейдите на **Keys**
2. Нажмите **Show Key** — UI запросит текущее значение ключа из OpenBao KV
3. Ключ отобразится в base64 — это 256-битный мастер-ключ Кузнечика

**Страница Keys — ротация ключа:**

1. Нажмите **Rotate Key** → подтвердите
2. UI сгенерирует новый 256-битный ключ (crypto/rand) и запишет в OpenBao KV
3. KMS подхватит новый ключ в течение 30 секунд (health check интервал)
4. Перешифруйте существующие секреты: `kubectl get secrets --all-namespaces -o json | kubectl replace -f -`

**Страница Secrets — просмотр деталей:**

1. Перейдите на **Secrets**, выберите namespace в фильтре
2. Кликните по любому секрету — откроется модальное окно с:
   - Namespace, Type, UID, ResourceVersion, дата создания
   - Labels и Annotations
   - Все data-ключи с base64-значениями (зашифрованные данные из etcd)
3. Чтобы расшифровать — скопируйте base64-значение и перейдите на **Decrypt**

### 6.6 Устранение проблем с UI

| Симптом | Диагностика | Решение |
|---|---|---|
| Pod в `ErrImageNeverPull` | `kubectl describe pod ...` → видно `pullPolicy: Never` | Загрузить образ: `docker save kubebao/kubebao-ui:dev \| rdctl shell -- docker load` |
| `curl http://localhost:8443` не отвечает | `kubectl get svc kubebao-ui -n kubebao-system` | Проверить, что port-forward запущен в отдельном терминале |
| `/api/status` возвращает `openbao: sealed` | `kubectl exec openbao-0 -n openbao -- bao status` | Выполнить unseal: `bao operator unseal $KEY` |
| `/api/status` возвращает `openbao: error` | `kubectl logs kubebao-ui ...` | Проверить `OPENBAO_ADDR` и сетевую доступность |
| Ingress возвращает 404 | `kubectl get ingress -n kubebao-system` | Проверить `ingressClassName`, добавить запись в `/etc/hosts` |
| Ротация ключа — ошибка 401 | Логи UI | Задать `OPENBAO_TOKEN` (раздел 6.3) |
...
**KMS (Key Management Service)** — это механизм Kubernetes для шифрования данных at rest в etcd. Без KMS все Kubernetes Secrets хранятся в etcd в **открытом виде** (base64 — это не шифрование). Любой, кто получит доступ к etcd-данным или бэкапу, увидит все секреты.

**Как это работает:**

1. Когда вы создаёте Secret через `kubectl create secret`, kube-apiserver отправляет данные KMS-плагину (`kubebao-kms`) через Unix-сокет
2. KMS-плагин шифрует данные алгоритмом **Кузнечик (ГОСТ Р 34.12-2015)** в режиме AEAD (CTR + CMAC по ГОСТ Р 34.13-2015)
3. В etcd сохраняется **зашифрованный** blob с маркером `k8s:enc:kms:v2:kubebao-kms`
4. При чтении через `kubectl get secret` — kube-apiserver автоматически запрашивает расшифровку у KMS-плагина

**Почему `kubectl` показывает данные в открытом виде:**

`kubectl get secret -o yaml` возвращает **расшифрованные** данные, потому что kube-apiserver прозрачно вызывает KMS-плагин для дешифровки перед отдачей клиенту. Это стандартное поведение Kubernetes — шифрование at rest защищает данные **в хранилище** (etcd, бэкапы), а не при передаче через API.

**Где данные зашифрованы, а где нет:**

| Уровень | Данные |
|---|---|
| etcd (на диске, бэкапы) | **Зашифрованы** Кузнечиком |
| kube-apiserver → клиент (kubectl) | **Расшифрованы** (API прозрачно расшифровывает) |
| KubeBao UI (Decrypt-раздел) | Позволяет вручную расшифровать blob из etcd мастер-ключом |
| OpenBao KV | Ключ шифрования хранится здесь, защищён seal-механизмом OpenBao |
...
### 8.1 Создание Kubernetes Secret обычным способом
...
### 8.2 Чтение секрета через kubectl (расшифровано)
...
### 9.1 Применение примера BaoSecret
...
### 9.3 Проверка созданного Kubernetes Secret
...
### 9.4 Проверка автоматического обновления
...
### 10.4 Полный демо (BaoSecret + CSI + синхронизация)
...
### 11.2 Проверка через etcdctl (если есть доступ)
...
### 12.1 Ротация ключа шифрования Кузнечик
...
### 12.2 Обновление версии KubeBao
